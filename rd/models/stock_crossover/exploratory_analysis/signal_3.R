...
gpd_quantile <- function(p_cond, u, scale, shape) {
  ...
  if (shape == 0) return(u - scale * log(p_cond))
...
predict_evt <- function(Xnew, probs=c(.95,.99,.995,.999)) {
  ...
  clusterExport(cl, varlist=c("fit_evt","keep_scale","keep_shape",
                              "x_names","u","prob_above_u","gpd_quantile",
                              "expected_shortfall","gpd_tail_prob","y","log"), envir=environment())
...
list(
  ...
  tail_probability = function(x_row, y0) {
    xdf <- as.data.frame(as.list(x_row)); names(xdf) <- x_names
    pred <- predict(fit_evt, newdata=xdf, type="response")
    scale_i <- pred$scale; shape_i <- pred$shape

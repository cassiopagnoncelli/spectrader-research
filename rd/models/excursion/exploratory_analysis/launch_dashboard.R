...
check_data <- function() {
  ...
  if (has_dfsr && has_posl && has_f_star) {
    ...
    cat("You can do this by running: source('rd/models/excursion/trade_simulation.R')\n\n")
    FALSE
  }
}
...
launch_dashboard <- function(load_data = FALSE) {
  if (load_data) {
    ...
    source("rd/models/excursion/trade_simulation.R")
  ...
  if (check_data()) {
    ...
    # Run the dashboard
    shiny::runApp("rd/models/excursion/dashboard.R", launch.browser = TRUE)
  } else {
...
if (!interactive()) {
  ...
  cat("       shiny::runApp('rd/models/excursion/dashboard.R')\n\n")
}
